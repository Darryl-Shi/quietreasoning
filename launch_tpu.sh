...
# Optionally set REPO_URL and WANDB_API_KEY environment variables.
...
for ((i=0; i<WORKER_COUNT; ++i)); do
  ...
  REMOTE_B64=$(REPO_URL="${REPO_URL}" WANDB_API_KEY="${WANDB_API_KEY}" CMD_CONTENT="${CMD}" WORKER_INDEX="${i}" PROCESS_COUNT="${WORKER_COUNT}" python3 - <<'PY'
